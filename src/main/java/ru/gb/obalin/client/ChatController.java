    ...
    private void showNotification() {
        final Alert alert = new Alert(Alert.AlertType.ERROR,
                "Не могу подключиться к серверу.\n" +
                        "Проверьте, что сервер подключен ",
                new ButtonType("Повторная попытка ", ButtonBar.ButtonData.OK_DONE),
                new ButtonType("Разрыв соединения ", ButtonBar.ButtonData.CANCEL_CLOSE));
